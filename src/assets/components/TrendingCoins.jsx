    ...
    <div className="w-full bg-bgBottomNavigation p-8 border-slate-800 rounded-2xl flex flex-col items-start">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-2 w-full my-5">
        {data.length > 0
          ? data.map((coin) => {
              return (
                <Link to={`/:coin${coin.name}`} key={coin.id} state={coin}>
                  <div className="flex flex-col items-start justify-center gap-2   bg-primaryColor bg-opacity-5 hover:bg-primaryColor transition-colors duration-300 p-4 text-headingColor rounded-md" >
